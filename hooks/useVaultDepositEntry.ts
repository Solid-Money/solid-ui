 ...
 * needs primed.
 *
 * Two flows, chosen by issuer (see {@link usesDirectSavingsDeposit}). They are not
 * two skins of one screen — they put money in by different routes, which is why
 * each needs its own setup:
 *
 * - The direct-deposit flow sends *new* money in: pick a currency, then a chain,
 *   then send to an address the backend mints for that pairing. So
 *   `depositFromSolid` is off and any address left from an earlier deposit is
 *   cleared, or the flow opens on a stale QR for a chain the user has moved off.
 * - The amount form moves a balance the user already holds in Solid, so
 *   `depositFromSolid` is on and it opens straight onto it.
 *
...
export const useVaultDepositEntry = (
  ...
  const isDirectDeposit = usesDirectSavingsDeposit(provider);
  ...
  }, [isDirectDeposit, vaultType]);

  return useMemo(
    () => ({
      modal: isDirectDeposit ? DEPOSIT_MODAL.OPEN_SAVINGS_FUND : DEPOSIT_MODAL.OPEN_FORM,
      onBeforeOpen,
    }),
    [isDirectDeposit, onBeforeOpen],
  );
};
