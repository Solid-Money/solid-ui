...
export function useAllTokens(showNativeToken = true) {
	...
	const mergedTokens = useMemo(() => {
		...
		return [...tokens].map(([, token]) => ({ ...token }));
	}, [allTokens, importedTokens, chainId, showNativeToken, tokensBlackList, tokenList, shouldUseCachedData, cachedTokens]);

	// Cache fresh data when it becomes available
	useEffect(() => {
		if (!loading && !loadingTokenList && allTokens && mergedTokens.length > 0 && !shouldUseCachedData) {
			setCachedTokens(chainId, mergedTokens);
		}
	}, [loading, loadingTokenList, allTokens, mergedTokens, chainId, shouldUseCachedData, setCachedTokens]);
	...
	return useMemo(
		() => ({
			...
			isLoading:
				loading ||
				loadingTokenList ||
				Boolean(allTokens && !mergedTokens.length),
		}),
