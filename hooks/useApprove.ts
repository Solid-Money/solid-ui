...
export function useBatchApproveAndSwap(
    ...
    const { data: approveConfig } = useSimulateContract({
        ...
        abi: erc20Abi,
        functionName: "approve",
        args: [ALGEBRA_ROUTER, amountToApprove ? BigInt(amountToApprove.quotient.toString()) : 0] as [Address, bigint],
    });
...
export function useBatchApproveAndPegSwap(
    ...
    const token = inputCurrencyAmount?.currency?.isToken
...
export function useApproveCallbackFromLendingPool(
    ...
    return useApprove(
        inputCurrencyAmount,
        lendingPoolAddress || MarketData.addresses.LENDING_POOL,
    );
}
