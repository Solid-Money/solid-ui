...
function usePegLiquidity(pegSwapAddress?: string, inputCurrency?: Currency, outputCurrency?: Currency) {
    ...
    const { data: liquidity } = useReadPegSwapGetSwappableAmount({ address, args: [source, target] });
...
function usePegSwapAddress(inputCurrencyAddress: Address | undefined, outputCurrencyAddress: Address | undefined) {
    return useMemo(() => {
        ...
        const swapPairs = [
            ...
            { addresses: [USDC_V2, USDC_STARGATE], pegSwap: STARGATE_PEGSWAP },
...
export default function usePegSwapCallback(
    ...
    const inputCurrencyAddress = inputCurrency instanceof Token ? (inputCurrency.address as Address) : undefined;
    const outputCurrencyAddress = outputCurrency instanceof Token ? (outputCurrency.address as Address) : undefined;
