...
const DepositToCard = () => {
  ...
  const isRain = provider === CardProvider.RAIN;
  const firstContractWithDeposit = contracts?.find(c => c.depositAddress);
  /** Bridge: funding_instructions.address; Rain: first contract depositAddress */
  const fundingAddress = useMemo((): Address | null => {
    if (isRain && firstContractWithDeposit?.depositAddress) {
      return firstContractWithDeposit.depositAddress as Address;
    }
    if (provider === CardProvider.BRIDGE && cardDetails?.funding_instructions?.address) {
      return cardDetails.funding_instructions.address as Address;
    }
    return null;
  }, [
    isRain,
    provider,
    firstContractWithDeposit?.depositAddress,
    cardDetails?.funding_instructions?.address,
  ]);
  const hasFundingAddress = !!fundingAddress;
  const isLoadingFunding = isRain && contractsLoading;
  ...
  const handleContinue = async () => {
    if (!selectedToken || !amount || !fundingAddress || !user) {
      return;
    }
    ...
    try {
      ...
      const clientTxId = await createActivity({
        ...
        metadata: {
          ...
          processingStatus: CardDepositProcessingStatus.SENDING,
      ...
      await updateActivity(clientTxId, {
        ...
        metadata: {
          ...
          processingStatus: isRain
            ? CardDepositProcessingStatus.AWAITING_RAIN
            : CardDepositProcessingStatus.AWAITING_BRIDGE,
        },
      });
  ...
  const isValidAmount =
    hasFundingAddress && amount && Number(amount) > 0 && Number(amount) <= availableBalance;
      ...
      <View className="mx-auto h-full w-full max-w-md">
        ...
        {isLoadingFunding ? (
          <View className="items-center py-12">
            ...
            <Text className="mt-4 text-muted-foreground">Loading deposit details...</Text>
          </View>
        ) : (
          <>
                ...
                {selectedToken ? (
                  <>
                    ...
                    <View className="ml-3 flex-1">
                      ...
                      <Text className="font-medium text-[#ACACAC]">
                    ...
                    <View className="flex-row items-center gap-4">
                      ...
                      <Pressable
                        ...
                        className="rounded-xl bg-[#4D4D4D] px-3 py-1"
                      >
            ...
            <Button
              ...
              disabled={!isValidAmount || isDepositing || isLoadingFunding}
            >
              <Text className="text-base font-bold">
                {isDepositing ? 'Depositing...' : 'Continue'}
              </Text>
