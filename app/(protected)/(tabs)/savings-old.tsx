/**
 * Savings page using pre-e4cba95 calculation (Total value = balance × rate + accrued interest).
 * Compare with /savings for side-by-side check.
 */
...
import SavingCountUpOld from '@/components/SavingCountUpOld';
...
import SavingVaultOld from '@/components/Savings/SavingVaultOld';
...
export default function SavingsOld() {
  const { user } = useUser();
  ...
  const displaySuffix = useMemo(() => {
    if (VAULTS[selectedVault].name === 'FUSE' || VAULTS[selectedVault].name === 'ETH') {
      return VAULTS[selectedVault].name === 'FUSE' ? 'FUSE' : 'ETH';
    }
    return null;
  }, [selectedVault]);
  ...
  const stickyHeader = (
    <View className="mx-auto w-full max-w-7xl px-4 pb-[40px] pt-6 md:pb-7 md:pt-12">
      {isScreenMedium ? (
        ...
        <Text className="text-3xl font-semibold">Savings (old)</Text>
      )}
      <View className="mt-2 rounded-lg border border-amber-500/50 bg-amber-500/10 px-3 py-2">
        <Text className="text-sm text-amber-200">
          Old calculation: Total value = balance × rate + accrued interest. Compare with{' '}
          <Link href="/savings">
            <Text className="font-semibold underline">/savings</Text>
          </Link>
        </Text>
      </View>
    </View>
        ...
        {isScreenMedium ? (
          <View className="flex-row gap-4">
            <View className="flex-col gap-4">
              {VAULTS.map(vault => (
                <SavingVaultOld key={vault.name} vault={vault} />
              ))}
            </View>
            <View
                ...
                <View className="flex-1 justify-between gap-12 border-b border-[#ffffff]/20 bg-transparent p-6 pb-10 md:gap-4 md:border-b-0 md:border-r md:px-10 md:py-8">
                  <View>
                    <View className="flex-row items-center gap-2 pb-1">
                      ...
                      <TooltipPopover text="Old: balance x rate + accrued interest (animation)" />
                    </View>
