...
export default function RewardsBenefitsScreenNew() {
  ...
  const content = TIER_CONTENT[selectedTier];
  const subtitle =
    rewardsData?.currentTier === selectedTier ? 'Your current tier' : content.unlockCopy;
  ...
  return (
    <PageLayout showNavbar={false} edges={['left', 'right', 'bottom']} additionalContent={header}>
      <View className="gap-6 pb-24" style={{ paddingTop: insets.top + HEADER_ROW_HEIGHT }}>
        <View className="items-center gap-1 pt-4">
          <View className="h-[320px] w-[320px] items-center justify-center">
            ...
            />
            <Image
              source={getAsset(TIER_HERO_ASSET[selectedTier])}
