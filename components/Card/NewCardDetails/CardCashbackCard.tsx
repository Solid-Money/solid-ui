 ...
 *
 * The figure and the fill count escrowed cashback alongside settled — see
 * `monthlyCashbackTotal`. Counting only what had settled left the line empty
 * for the fortnight an escrow takes to mature, which is exactly when a
 * cardholder who just spent comes looking for it.
 */
...
const CardCashbackCard = ({ onPress }: CardCashbackCardProps) => {
  ...
  const earned = monthlyCashbackTotal(
  ...
  // same cap when it matures, so it counts against the target too — otherwise
  // this asks for spend that will be capped.
...
const styles = StyleSheet.create({
  ...
  fill: { backgroundColor: CASHBACK_EARNED_COLOR, borderRadius: 2.5, height: 5 },
