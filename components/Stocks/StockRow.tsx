...
export default function StockRow(props: StockRowProps) {
  ...
  return (
    <Pressable onPress={onPress} className="flex-row items-center gap-3 p-3 active:opacity-70">
      ...
      {props.variant === 'holding' && (
        <View className="items-end gap-0.5">
          ...
          <Text className="text-xs text-[#808080]">{props.shares.toFixed(4)} shares</Text>
        </View>
      )}
    </Pressable>
  );
