...
type SavingsFundDepositAddressProps = {
...
/**
 * Step 3 of the savings funding flow — the deposit address, QR and network
 * summary, polling for the deposit so the user is handed to its progress screen
 * the moment the transfer is seen on chain.
 */
const SavingsFundDepositAddress = ({
  address,
  symbol,
  chainId,
  onChangeNetwork,
  ...
  const network = useMemo(
    () => getSavingsFundNetworks(symbol).find(item => item.chainId === chainId),
    [symbol, chainId],
  );
  ...
  return (
    <View className="gap-y-4">
      ...
      <Pressable
        ...
        onPress={() => Linking.openURL(SAVINGS_FUND_LEARN_URL)}
      >
...
export default SavingsFundDepositAddress;
