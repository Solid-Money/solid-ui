/**
 * Same as SavingVault but uses useVaultTotalValueOld (balance + interest).
...
interface SavingVaultOldProps {
  vault: Vault;
}

const SavingVaultOld = ({ vault }: SavingVaultOldProps) => {
  ...
  const { data: vaultBalance } = useVaultTotalValueOld(vault);
